...
BEGIN
    ...
    SELECT COUNT(*) INTO QUANTITY_PURCHASE FROM PURCHASE;
		...
		IF(QUANTITY_PURCHASE=(SELECT COUNT(*) FROM PURCHASE)) THEN
			SELECT CONCAT('Purchase can not be done') AS MENSAJE;
		ELSE 
			SELECT CONCAT('Purchase correctly done!') AS MENSAJE;
...
CALL REGISTER_PURCHASE ('client2', 'Civic', 2, '2025-03-20', 10);
...
BEGIN
    ...
    WHILE END_C = 0 DO
		SELECT CONCAT('NAME MODEL: ',  NAME_M, ' PRICE: ', PRICE_M, ' QUANTITT: ', STOCK_M) AS MENSAJE;
        FETCH C INTO NAME_M, PRICE_M, STOCK_M ;
	END WHILE;
    CLOSE C;
END //
...
DELIMITER ;
SELECT COUNT_TOTAL_CARS_BY_CARDEALERSHIP(3);

-- P7 SHOWS ALL THE PURCHASES REGISTERED IN A CARDELAERSHIP (SI NO SE CORRIGEN LOS HANDLERS HAY QUE QUITAR)
DELIMITER //
