...
-- HAY QUE CAMBIAR PARA QUE PUEDA COMPRAR MAS DE UNA VEZ EL MISMO MODELO
DELIMITER //
...
BEGIN
    ...
    SELECT COUNT(*) INTO QUANTITY_PURCHASE FROM PURCHASE;
		...
		INSERT INTO PURCHASE (DNI, NAME_MODEL, ID_CAR_DEALER, DATE_PURCHASE, QUANTITY)
		VALUES 
		...
		IF(STOCK_BEFORE>(SELECT STOCK FROM MODEL)) THEN
			SELECT CONCAT('No se ha podido actualizar el stock') AS MENSAJE;
		ELSE 
			SELECT CONCAT('Stock actualizado correctamente') AS MENSAJE;
		END IF;
